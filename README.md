...
extension — e.g. `00011/K_楷体`. List them with `reader.keys()`.
...
`"00011"`). `reader.character()` / `reader.characters()` return labels **only if
the dataset was built with a character map** — the bundled build ships **without
one**, so they return `undefined`. The repo's `Key&Value.json` is _not_ used: it
uses a different ID numbering than the image folders (it maps 㐁 to `"00001"`,
but that glyph lives in folder `"00011"`), so joining it by folder name would
attach the wrong characters. The filename part of a key still encodes the script
style — `K_` regular, `O_G_` oracle-bone, `J_` bronze, etc. — and often contains
the character itself. To embed labels, build with a map keyed to the folder IDs
(see [Building your own dataset](#building-your-own-dataset)).
...
- `--characters <path>` — ID → character JSON embedded as labels (default: none).
  The map must be keyed by the image **folder IDs**; only IDs present in both the
  map and the dataset are kept. The repo's `Key&Value.json` does _not_ match the
  folder numbering, so it is not used by default.
